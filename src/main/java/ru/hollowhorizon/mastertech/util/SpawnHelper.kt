...
object SpawnHelper {
    ...
    private fun WorldServer.getSafeSpawn(position: BlockPos): BlockPos {
        var d0 = -1.0
        ...
        for (j2 in xPos - 16..xPos + 16) {
            ...
            for (l2 in zPos - 16..zPos + 16) {
                ...
                heightLoop@ while (height >= 0) {
                    if (isAirBlock(mutable.setPos(j2, height, l2))) {
                        while (height > 0 && isAirBlock(mutable.setPos(j2, height - 1, l2))) --height
                            ...
                            for (j4 in 0..2) {
                                for (k4 in 0..3) {
                                    for (l4 in -1..3) {
                                        ...
                                        mutable.setPos(i5, j5, k5)
                                        if (l4 < 0 && !getBlockState(mutable).material.isSolid || l4 >= 0 && !isAirBlock(
                                                mutable
                                            )
                                        ) {
                                            --height
        ...
        if (d0 < 0.0) {
            for (l5 in xPos - 16..xPos + 16) {
                ...
                for (j6 in zPos - 16..zPos + 16) {
                    ...
                    var height: Int = height - 1
                    heightLoop@ while (height >= 0) {
                        if (isAirBlock(mutable.setPos(l5, height, j6))) {
                            while (height > 0 && isAirBlock(mutable.setPos(l5, height - 1, j6))) --height
                            ...
                            for (l7 in i2 until i2 + 2) {
                                ...
                                for (i10 in 0..3) {
                                    for (k10 in -1..3) {
                                        ...
                                        val j11 = height + k10
                                        val k11 = j6 + (i10 - 1) * k9
                                        mutable.setPos(i11, j11, k11)
                                        if (k10 < 0 && !getBlockState(mutable).material.isSolid || k10 >= 0 && !isAirBlock(
                                                mutable
                                            )
                                        ) {
                                            --height
        ...
        if (d0 < 0.0) {
            yPosCopy = MathHelper.clamp(yPosCopy, 70, height - 10)
            k2 = yPosCopy
            ...
            for (j7 in -1..1) {
                for (i8 in 1..2) {
                    for (i9 in -1..2) {
                        ...
                        val flag = i9 < 0
                        mutable.setPos(l9, j10, l10)
                        setBlockState(
                            mutable,
                            if (flag) Blocks.OBSIDIAN.defaultState else Blocks.AIR.defaultState
                        )
                    }
                }
            }
        ...
        for (k7 in -1..2) {
            for (j8 in -1..3) {
                if (k7 == -1 || k7 == 2 || j8 == -1 || j8 == 3) mutable.setPos(i6 + k7 * l6, k2 + j8, k6 + k7 * i3)
            }
        }

        for (k8 in 0..1) {
            for (j9 in 0..2) mutable.setPos(i6 + k8 * l6, k2 + j9 - 2, k6 + k8 * i3)
        }
